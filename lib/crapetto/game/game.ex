defmodule Crapetto.Game do
  ...
  def new_game(id_owner, owner) do
    id_game =
      Enum.reduce(1..6, "", fn _, acc ->
        Enum.random(String.graphemes("ABCDEFGHIJKLMNOPQRSTUVWXYZ23456789")) <> acc
      end)
  ...
  def remove_player(
        %Crapetto.Game{status: :starting, players: players, locked_to_join: locked} = game,
        a_player
      ) do
    if locked do
      game
  ...
  defp reshuffle(
         %{ligretto: ligretto, deck: deck, displayed: displayed, series: series} = player_deck
       ) do
    new_deck =
  ...
  def lock_player(
        %Crapetto.Game{players: players, players_lock: players_lock} = game,
        player,
        countdown
      ) do
  ...
  def deck_to_ligretto(game, player) do
    ...
    new_player_deck = %{
      player_deck
      | deck: deck,
        ligretto: if(card == nil, do: ligretto, else: [card | ligretto])
    }
  ...
  defp ligretto_to_series(game, player, location) do
    ...
    if series[location] != nil do
      ...
      game = %{
        game
        | players_decks: Map.put(game.players_decks, player, new_player_deck),
  ...
  def show_three(game, player) do
    ...
    {displayed, deck} =
      if count_deck == 0, do: {[], Enum.reverse(displayed)}, else: {displayed, deck}
  ...
  def play_on_stack(game, {_, color, number} = card) do
    if number == 1 do
      ...
      %{
        game
        | stacks: Map.put(game.stacks, stack, [card | stack_content]),
          last_stack_played: stack
      }
    end
  end
  ...
  defp fill_ligretto(game) do
    Enum.reduce(game.players, game, fn player, game1 ->
      Enum.reduce(1..(10 + game.series), game1, fn _, g -> deck_to_ligretto(g, player) end)
    end)
  end

  defp fill_series(game) do
    Enum.reduce(game.players, game, fn player, game1 ->
      Enum.reduce(1..game.series, game1, fn location, g ->
        ligretto_to_series(g, player, location)
      end)
    end)
  end

  def start_game(
        %Crapetto.Game{players: players, num_players: num_players, players_scores: players_scores} =
          game
      ) do
    players_scores =
    ...
    players_decks =
      ...
      |> Enum.map(fn p ->
        {p,
         %{
           deck:
             for(c <- [:red, :green, :blue, :yellow], n <- 1..10, do: {p, c, n}) |> Enum.shuffle(),
           ligretto: [],
  ...
  def update_score(
    ...
    last_play_score =
      ...
      |> Enum.map(fn player ->
        {player,
         {count_stacks[player], ligrettos[player], count_stacks[player] - 2 * ligrettos[player]}}
      end)
      |> Map.new()
    ...
    players_scores =
      players_scores
      |> Enum.map(fn {player, previous_score} ->
        {player, previous_score + last_score_only[player]}
      end)
      |> Map.new()
    ...
    if score >= game.score_to_win do
      ...
      %Crapetto.Game{game | players_scores: players_scores, last_play_score: last_play_score}
    end
  end
